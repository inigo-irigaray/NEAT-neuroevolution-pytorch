...
class FeedForwardNet():
    def __init__(self, n_in, n_out, in2out, out_response, out_bias, batch_size=1, act=sigmoid_act, dtype=torch.float64):

        self.n_in = n_in
        self.n_out = n_out
        self.in2out = torch.tensor((n_out, n_in)), in2out, dtype=dtype)
        self.out_response = torch.tensor(out_responses, dtype=dtype)
        self.out_bias = torch.tensor(out_biases, dtype=dtype)
        self.act = act
        self.dtype = dtype
