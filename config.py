...
class ConfigParameter:
  ...
  def interpret(self, config_dict):
    ...
    try:
      ...
      raise RuntimeError("Error interpreting config item {} with value {!r} and type {}".format(self.name, value, self.value_type))
...
class Config:
  ...
  __params = [ConfigParameter('pop_size', int), ConfigParameter('fitness_criterion', str), 
              ConfigParameter('fitness_threshold', float), ConfigParameter('reset_on_extinction', bool),
    ...
    for p in self.__params:
      if p.default is None:
        ...
        try:
          ...
          warnings.warn("Using default {!r} for '{!s}'".format(p.default, p.name), DeprecationWarning)
      param_list_names.append(p.name)
